...
//				populateValidationDataObject(requestTrackerId, kanbanJiraIssueList, validationDataMap, projectName);
	 ...
	 * @param validationKey
	 */
	private void populateValidationDataObject(String requestTrackerId, List<KanbanJiraIssue> epicList,
		...
		if (requestTrackerId.toLowerCase().contains(KPISource.EXCEL.name().toLowerCase())) {
			...
			epicList.forEach(el -> {
				if (!el.getProjectName().isEmpty()) {
					...
					LocalDate dateValue = LocalDate.parse(el.getChangeDate().split("\\.")[0], DATE_TIME_FORMATTER);
					dateLists.add(dateValue.getYear() + Constant.DASH + dateValue.getMonth().getValue());
				}
			});

			ValidationData validationData = new ValidationData();
			validationData.setMonthList(dateLists);
