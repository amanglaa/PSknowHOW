...
db.createUser({user: '${MONGODB_ARCHIVE_USER}', pwd: '${MONGODB_ARCHIVE_PASS}', roles:[{role:'readWrite', db:'${MONGODB_ARCHIVE_DATABASE}'}]})
EOF
  if [ $? -eq  0 ]
  then
      echo "=> ${MONGODB_ARCHIVE_DATABASE} user has been created in MongoDB" >> ${LOG_DIR}/create_user.log 2>&1
  else
      echo "=> ${MONGODB_ARCHIVE_DATABASE} user is not created in MongoDB" >> ${LOG_DIR}/create_user.log 2>&1
      exit 1
  fi
fi
...
echo "=> Creating an ${MONGODB_BACKUP_DATABASE} user with a password in MongoDB" >> ${LOG_DIR}/create_user.log 2>&1
    mongo admin -u ${MONGODB_ADMIN_USER} -p ${MONGODB_ADMIN_PASS} << EOF
use ${MONGODB_ARCHIVE_DATABASE}
db.createUser({user: '${MONGODB_BACKUP_USER}', pwd: '${MONGODB_BACKUP_PASS}', roles:[{role:'readWrite', db:'${MONGODB_BACKUP_DATABASE}'}]})
EOF
  if [ $? -eq  0 ]
  then
      echo "=> ${MONGODB_ARCHIVE_DATABASE} user has been created in MongoDB" >> ${LOG_DIR}/create_user.log 2>&1
