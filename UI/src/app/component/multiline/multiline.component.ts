    ...
    draw(status) {
        ...
        for (const i in data) {
            for (let j = 0; j < data[i].value.length; j++) {
                data[i].value[j].xName = data[i]?.value[j]?.hasOwnProperty('xAxisTick') ? data[i]?.value[j]?.xAxisTick : (j + 1);
...
maxYValue += divisor;
        ...
        const svgX = d3.select(this.elem).select('#horizontalSVG').append('svg')
        ...
        const div = d3.select(this.elem).select('#multiLineChart').append('div')
            .attr('class', 'tooltip')
            .style('display', 'none')
            .style('opacity', 0);
 ...
 d3.select(this).call(transition);
        ...
        svgX.select('.x').selectAll('.tick').each(function(dataObj) {
            ...
            tick.attr('transform', 'translate(' + translate[0] + ',' + translate[1] + ')');
            if (dataObj === 1) {
                ...
                // }
                svgX.select('.lines').attr('transform', 'translate(' + translate[0] + ',' + translate[1] + ')');
            }
        });
